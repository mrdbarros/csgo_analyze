...
for i,tab_file in enumerate(tabular_files):
    ...
    if not os.stat(tab_file).st_size == 0 and os.path.isfile(tab_file.parent/"winner.txt"):
        ...
        new_csv['related_image'] = str(tab_file.parent)+"/output_map"+new_csv['index'].astype(str).str.pad(width=2,fillchar="0")+".jpg"
...
class CSGORoundsDataset(torch.utils.data.Dataset):
    def __init__(self,image_paths:list,round_paths:list,tabular_x_data:pd.DataFrame,round_winners,image_transform=None,cat_transform=None,
        ...
        cont_columns = [column for column in tabular_x_data.columns if not column[column.rfind("_")+1:] in x_category_map]
...
image_transforms= torchvision.transforms.Compose([torchvision.transforms.Resize(200),torchvision.transforms.ToTensor(),
