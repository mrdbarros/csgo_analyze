...
device = "cuda:0"
logging.basicConfig(filename='sketch.log', level=logging.INFO)

image_files = data_loading.get_files(path, extensions=['.jpg'])
tabular_files = data_loading.get_files(path, extensions=['.csv'])


randomlist = random.sample(range(len(tabular_files)), 500)
...
train_dl_mixed = torch.utils.data.DataLoader(train_dataset, batch_size=bs, shuffle=True, num_workers=4,
                                             collate_fn=data_loading.snapshot_sequence_batching(seq_size), drop_last=True)
valid_dl_mixed = torch.utils.data.DataLoader(valid_dataset, batch_size=bs, shuffle=True, num_workers=4,
                                             collate_fn=data_loading.snapshot_sequence_batching(seq_size), drop_last=True)
...
iter_train = iter(train_dl_mixed)
...
model.load_state_dict(torch.load(os.path.expanduser('~/projetos/data/csgo_analyze/processed_test/full_model-unfrozen-seq_model0.pt')))
