...
cont_transforms = data_loading.TransformPipeline(
    [data_loading.Normalize(valid_tabular.iloc[:, :-2], category_map=x_category_map), data_loading.ToTensor])
...
valid_dataset = data_loading.CSGORoundsDataset(valid_images, valid_rounds, valid_x_tabular, valid_winners,
                                               ...
                                               y_category_map, batch_transform,is_eval=is_eval)

valid_dl_mixed = torch.utils.data.DataLoader(valid_dataset, batch_size=bs, shuffle=True, #num_workers=6,
                                             collate_fn=data_loading.pad_snapshot_sequence(seq_size,is_eval=is_eval), drop_last=True)


print(next(iter(valid_dl_mixed)))
...
# seq_model.load_state_dict(torch.load(os.path.expanduser('~/projetos/data/csgo_analyze/processed_test/seq_model.pt')))
...
model.load_state_dict(torch.load(os.path.expanduser('~/projetos/data/csgo_analyze/processed_test/full_model-unfrozen-seq_model1.pt')))
...
training.validation_epoch(valid_dl_mixed,model,loss_fn,tensorboard_class)
